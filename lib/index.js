...
var ReactFlagsSelect = function (_Component) {
	...
	ReactFlagsSelect.prototype.toggleOptions = function toggleOptions(event) {
	...
	ReactFlagsSelect.prototype.closeOptions = function closeOptions(event) {
		if (event.target !== this.refs.selectedFlag && event.target !== this.refs.flagOptions && event.target !== this.refs.filterText && event.target !== this.refs.countryFlag && event.target !== this.refs.countryFlagImg && event.target !== this.refs.countryLabel && event.target !== this.refs.carat) {
			this.setState({
				openOptions: false
			});
		}
		...
		return _react2.default.createElement(
			...
			_react2.default.createElement(
				...
				{ ref: 'selectedFlag', style: { fontSize: selectedSize + 'px' }, className: 'selected--flag--option ' + (this.props.disabled ? 'no--focus' : ''), tabIndex: '0', onClick: this.toggleOptions, onKeyUp: function onKeyUp(evt) {
						return _this3.toggleOptionsWithKeyboard(evt);
					} },
				isSelected && _react2.default.createElement(
					'span',
					{ ref: 'countryFlag', className: 'country-flag', style: { width: selectedSize + 'px', height: selectedSize + 'px' } },
					_react2.default.createElement('img', { ref: 'countryFlagImg', src: require('../flags/' + isSelected.toLowerCase() + '.svg') }),
					this.props.showSelectedLabel && _react2.default.createElement(
						'span',
						{ className: 'country-label', ref: 'countryLabel' },
				...
				_react2.default.createElement(
					'span',
					{ ref: 'carat', className: 'arrow-down ' + (this.props.disabled ? 'hidden' : '') },
					'\u25BE'
				)
			),
			this.state.openOptions && _react2.default.createElement(
				...
				(this.state.filter ? this.state.filteredCountries : this.state.countries).map(function (countryCode) {
					return _react2.default.createElement(
						'div',
						{ className: 'flag-option ' + (_this3.props.showOptionLabel ? 'has-label' : ''), key: countryCode, tabIndex: '0', onClick: function onClick() {
								return _this3.onSelect(countryCode);
							}, onKeyUp: function onKeyUp(evt) {
								return _this3.onSelectWithKeyboard(evt, countryCode);
							} },
						_react2.default.createElement(
							'span',
...
process.env.NODE_ENV !== "production" ? ReactFlagsSelect.propTypes = {
